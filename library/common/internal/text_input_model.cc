...
void TextInputModel::AddString(std::string string) {
  EraseSelected();
  std::cout << state_.selection.begin << std::endl;
...
bool TextInputModel::MoveCursorUp() {
  ...
  size_t new_location = state_.selection.begin - previous_break + before_previous;
...
void TextInputModel::MarkText(Range range) { state_.composing = range; }

void TextInputModel::SelectText(Range range) { state_.selection = range; }
